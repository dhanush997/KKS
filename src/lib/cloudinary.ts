 ...
 * Uses a local disk storage fallback if Cloudinary credentials are not provided.
 * 
...
export async function uploadToCloudinary(fileStr: string, folder: string = "fashion-store"): Promise<string> {
  ...
  // Fallback to direct base64 database storage if Cloudinary isn't configured
  if (!cloudName || !apiKey || !apiSecret || cloudName === "your_cloudinary_cloud_name") {
    console.warn("Cloudinary credentials are not set. Saving image as base64 string directly in DB.");
    return fileStr; // Returns the base64 string directly (natively supported by browsers and Next.js Image with unoptimized flag)
  }
